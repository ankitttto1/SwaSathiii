...
export default function ResultCard({ result, onReset }: Props) {
  const confidenceColor =
    ...
    result.confidence >= 0.5 ? 'text-yellow-600' : 'text-red-500';

  const confidenceBg =
    result.confidence >= 0.8 ? 'bg-green-100' :
    result.confidence >= 0.5 ? 'bg-yellow-100' : 'bg-red-100';
  ...
  return (
    <div className="animate-scale-in">
      ...
      {/* Description */}
      <div className="bg-white rounded-2xl p-5 mb-4 border border-green-100 shadow-sm">
        <h4 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-2 flex items-center gap-2">
          <span className="w-5 h-5 bg-green-100 rounded-full flex items-center justify-center text-green-600 text-xs">i</span>
          About this waste
        </h4>
      ...
      {/* Recycling Tips */}
      <div className="bg-white rounded-2xl p-5 mb-5 border border-green-100 shadow-sm">
      ...
      <div className="bg-white rounded-2xl p-5 mb-5 border border-green-100 shadow-sm">
        ...
        <div className="flex items-center justify-center gap-6">
          <div className="text-center">
            ...
            <div className="text-sm font-bold text-gray-700">{result.label}</div>
            <div className="text-xs text-gray-400 mt-0.5">{result.binLabel}</div>
          </div>
        </div>
      </div>
      ...
      <button
        onClick={onReset}
        className="w-full py-3.5 bg-green-600 hover:bg-green-700 active:scale-95 text-white font-bold rounded-2xl transition-all duration-150 shadow-md shadow-green-200"
      >
        Scan Another Item
      </button>
    </div>
